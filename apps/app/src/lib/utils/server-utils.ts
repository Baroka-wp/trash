...
export const handleNextApiError = (error: unknown) => {
  if (error instanceof ZodError) {
    ...
    try {
      ...
      const errorCode = error.code as keyof typeof ERROR_CODES_BY_KEY
      const statusCode = ERROR_CODES_BY_KEY[errorCode]

      return NextResponse.json({ error: content }, { status: statusCode })
    } catch (e) {
      const errorCode = error.code as keyof typeof ERROR_CODES_BY_KEY
      const statusCode = ERROR_CODES_BY_KEY[errorCode]
