...
_buildnrun:
ifeq ($(CMD),app)
	docker build -f apps/app/Dockerfile -t trash/app --network host .
	docker run -e PORT=8080 --network host trash/app
else ifeq ($(CMD),landing)
	docker build -f apps/landing/Dockerfile -t trash/landing --network host .
	docker run -e PORT=8080 --network host trash/landing
else ifeq ($(CMD),cron)
	docker build -f apps/cron/Dockerfile -t trash/cron --network host .
	docker run --network host trash/cron
else
